...
// закешированной старой ссылкой); канонический путь теперь /stol.
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params, next }) => {
  ...
  // SPA-route navigation: если браузер просит HTML, отдаём фронт (StolPanel).
  if (request.method === "GET" && accept.includes("text/html") && !accept.includes("application/json")) {
    return next()
  }

  const targetUrl = path
    ? `${BACKEND_BASE}/stol/${path}${url.search}`
    : `${BACKEND_BASE}/stol${url.search}`
