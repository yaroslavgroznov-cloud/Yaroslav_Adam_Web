// Cloudflare Pages Function — reverse-proxy для /auth/* (Google OAuth F.69).
// Ring 6 (2026-06-24): /auth/google/start + /auth/google/status + /auth/google/revoke.
//
// ВАЖНО: callback идёт от Google НЕПОСРЕДСТВЕННО на adam-api.groznov.uk
// (settings в Google Console), не через этот proxy. После успешного
// callback'а backend редиректит на /?google_oauth=ok — CF Pages его обработает.
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  const targetUrl = `${BACKEND_BASE}/auth/${path}${url.search}`;
