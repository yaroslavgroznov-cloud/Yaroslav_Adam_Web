...
// CF Pages Function — proxy /payments/* на adam-api backend.
// F.42, 2026-05-28.
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params, next }) => {
  ...
  const targetUrl = path ? `${BACKEND_BASE}/payments/${path}${url.search}` : `${BACKEND_BASE}/payments${url.search}`

  // Webhook от Stripe идёт БЕЗ CF Access — оставляем требование email
  // только для остальных endpoint'ов.
  ...
  if (email) await setProxyAuthHeaders(forwardHeaders, env.ADAM_PROXY_SECRET, email)
