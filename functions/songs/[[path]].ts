...
// email пробрасывается как X-Adam-User-Email.
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params, next }) => {
  ...
  // Let the SPA handle HTML navigation
  if (request.method === "GET" && accept.includes("text/html") && !accept.includes("application/json")) {
    return next()
  }
  const targetUrl = path
    ? `${BACKEND_BASE}/songs/${path}${url.search}`
    : `${BACKEND_BASE}/songs${url.search}`
  ...
  if (!email) {
    return new Response(JSON.stringify({ detail: "No CF Access identity" }), {
      status: 401,
      headers: { "content-type": "application/json" },
    })
  }
  if (!env.ADAM_PROXY_SECRET) {
    return new Response(JSON.stringify({ detail: "ADAM_PROXY_SECRET not configured" }), {
      status: 500,
      headers: { "content-type": "application/json" },
    })
  }
