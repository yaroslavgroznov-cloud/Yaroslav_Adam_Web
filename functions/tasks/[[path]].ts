// Cloudflare Pages Function — reverse-proxy для /tasks/*.
// Sprint F.18, 2026-05-26.
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params, next }) => {
  ...
  // FastAPI: /tasks (root) — без trailing slash. Если path пуст — не добавляем "/".
  const targetUrl = path
    ? `${BACKEND_BASE}/tasks/${path}${url.search}`
    : `${BACKEND_BASE}/tasks${url.search}`;
