...
- Backend: `https://api.adam.groznov.uk/adam/*` (через Cloudflare Tunnel,
  репо `Yaroslav_Shtab`).
...
- `api.adam.groznov.uk` → **CF Tunnel** → backend `Yaroslav_Shtab/app/routers/adam_external.py`

`.env.production` уже содержит `VITE_ADAM_API_BASE=https://api.adam.groznov.uk`.
...
5. После первого выката: **Settings → Custom domains → add `adam.groznov.uk`**.
   ...
   соглашайся: tunnel теперь обслуживает `api.adam.groznov.uk`, а
   `adam.groznov.uk` должен указывать на Pages-проект.
6. **Cloudflare Access** → **Applications** → `Adam External` → **Edit**:
   - В Application domain добавь второй домен **`api.adam.groznov.uk`**.
