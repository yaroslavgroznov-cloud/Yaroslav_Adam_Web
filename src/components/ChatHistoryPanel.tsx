...
function gruppa(iso: string): string {
  ...
  if (sutki <= 0) return 'Сегодня'
  if (sutki === 1) return 'Вчера'
  if (sutki <= 7) return 'За неделю'
  if (sutki <= 30) return 'За месяц'
  return 'Раньше'
}

const PORYADOK = ['Сегодня', 'Вчера', 'За неделю', 'За месяц', 'Раньше']
...
export function ChatHistoryPanel({
  ...
  const sgruppirovano = useMemo(() => {
    const m = new Map<string, ConversationBrief[]>()
    for (const it of items ?? []) {
      const g = gruppa(it.last_message_at)
      if (!m.has(g)) m.set(g, [])
      m.get(g)!.push(it)
    }
    return PORYADOK.filter((g) => m.has(g)).map((g) => ({ title: g, rows: m.get(g)! }))
  }, [items])
  ...
  return (
    <div className="fixed inset-0 z-50 flex" style={tokeny}>
      ...
      >
          ...
          {!error && items !== null && items.length === 0 && (
            <div className="flex flex-col items-center mt-10 px-6 text-center">
              ...
              <p className="italic mt-2" style={{ fontSize: '14px', color: muted, maxWidth: 260 }}>
                Прошлых бесед пока нет. Нить начинается здесь.
              </p>
            </div>
          )}
        ...
        >
          Закрытая беседа не стирается: она уходит в долгую память Адама и
          остаётся здесь.
        </footer>
      </aside>
    </div>
